<h2 class="margin-bottom-1"><?php echo __('Edit article', 'article'); ?></h2>
...
<?php
    echo (
        ...
        Form::hidden('article_img', $imgname)
    );
?>
...
<div class="tab-content tab-page margin-bottom-1">
    ...
    <div class="tab-pane <?php if (Notification::get('settings')) { ?>active<?php } ?>" id="settings">
        ...
        <div class="form-group">
            <?php 
                if(file_exists($imgdir . $imgname) and !empty($imgname)) {
                    echo (
                        Form::label('file', __('Image', 'article').
                        ' / <a href="index.php?id=article&action=edit&article_id='.$article_id.'&delete_img='.$imgname.'&token='.Security::token().'" onClick="return confirmDelete(\''.__('Delete image', 'article').'\');">'.__('Delete', 'article').'</a>')
                    );
                    echo '<a href="'.$imgurl.'"><img src="'.$imgurl.'" alt="" style="height:30px;"/></a> &nbsp;';
                    echo '<div style="display:none;">'.Form::input('file', null, array('type' => 'file', 'size' => '25')).'</div>';
...
<div class="row margin-top-1">
    <div class="col-sm-6">
        <?php
            echo (
                Form::submit('edit_article_and_exit', __('Save and exit', 'article'), array('class' => 'btn btn-primary')).Html::nbsp(2).
                Form::submit('edit_article', __('Save', 'article'), array('class' => 'btn btn-primary')).Html::nbsp(2).
